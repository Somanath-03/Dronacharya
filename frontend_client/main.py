...
def identi_box(name,x,y,w,h,frame):
        ...
        try:
            font = ImageFont.truetype("arial.ttf", 25)  # Try loading the font
        except IOError:
...
def video_stream():
    while True:
        ...
        if not ret:
            ...
            global new_face_tracker
            if add_new_face:
                if not new_face_tracker:
                    print("initated tracking for new face")
                    bbox = cv2.selectROI("Tracking", frame, False)
                    initiate_tracking(bbox)
                    new_face_tracker = True
                    frame = new_face(frame,bbox)
                else:
                    frame = new_face(frame,bbox)
            else:
                frame = trained_face(frame)
...
def trained_face(frame):
    ...
    for (x, y, w, h) in faces:
        ...
        frame = identi_box("Unknown",x,y,w,h,frame)
    ...
    response = connection.recv_match(type ="VFR_HUD",blocking=True)
    print(response,response.alt)
    
    drawing_meter(60,60,35,150,390,3,70)    # speed_bk left most meter
    drawing_meter(60,60,35,150,180+int(response.airspeed),3,255,1,"Speed")   # speed writer left most meter
    
    drawing_meter(150,60,35,150,390,3,70)    # rpm_bk 2 - left most meter
    drawing_meter(150,60,35,150,180+int(response.groundspeed),3,255,1,"Groundspeed")   # rpm writer 2 - left most meter   
    
    drawing_meter(640-60,60,35,150,390,3,70)    # speed_bk right most meter
    drawing_meter(640-60,60,35,150,180+int(response.throttle),3,255,1,"Throttle")   # speed writer right most meter
    
    drawing_meter(640-150,60,35,150,390,3,70)    # rpm_bk 2 - right most meter
    drawing_meter(640-150,60,35,150,180+int(response.alt-584),3,255,1,"Altitude")   # rpm writer 2 - right most meter 
...
def new_face(frame,bbox):
    ...
    drawing_meter(640-150,60,35,150,390,3,70)    # rpm_bk 2 - right most meter
    drawing_meter(640-150,60,35,150,180+int(response.alt),3,255,1,"Altitude")   # rpm writer 2 - right most meter 
...
try:
  add_new_face = sys.argv[1].capitalize()

except IndexError:
  print("Error: No agument provided going to default mode")
  add_new_face = False  # Set a default value (optional)
new_face_tracker = False
...
with open("./face-trainner_2.yml", "r") as f:
    print("successfully found the file")
recognizer.read("./face-trainner_2.yml")
 
cap = cv2.VideoCapture(0) #Get video feed from the Camera

connection = mavutil.mavlink_connection('udpin:192.168.224.116:14551')
